...
oc process -f "${DIR}/rhods/odh-dashboard-config.yaml" | oc delete -n redhat-ods-applications -f -
oc rollout restart deployment/odh-dashboard -n redhat-ods-applications

oc process -f "${DIR}/rhods/rhods-groups-config-original.yaml" | oc apply -n redhat-ods-applications -f -
